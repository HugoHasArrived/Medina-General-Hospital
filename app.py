...
def new_count():
    conn = db()
    value = conn.execute("SELECT COUNT(*) AS count FROM assistance_requests WHERE status='New'").fetchone()["count"]
    conn.close()
    return value
...
def staff_dashboard():
    ...
    active = conn.execute("SELECT COUNT(*) FROM assistance_requests WHERE status!='Resolved'").fetchone()["count"]
    resolved = conn.execute("SELECT COUNT(*) AS count FROM assistance_requests WHERE status='Resolved'").fetchone()["count"]
    conn.close()
    cards = []
    for r in rows:
